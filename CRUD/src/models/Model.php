<?php
    ...
    abstract class Model{
        public static function All() : array{ // 'select()' Mètode per obtenir totes les files de la taula
            try{
                $db = new Database();
        ...
        public static function AllDepartment() : array{ // 'select()' Mètode per obtenir totes les files de la taula
            try{
                $db = new Database();
                $db->dbConnect("C:/temp/config.db"); // Realitzar la connexió a la base de dades - Carregar la connexió a la base de dades
                $table = static::$table; // Obtenir el nom de la taula de la classe filla
