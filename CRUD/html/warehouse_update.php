...
function getWarehouse($warehouses, $warehouse_id){ //Obtiene el Warehouse por su $_GET["id"]
...
try {
    ...
    $warehouse_name = $warehouse->getWarehouseName(); //Obtengo los valores del Warehouse.
    $location_id = $warehouse->getLocationId();
	$warehouse_spec = $warehouse->getWarehouseSpec();
	$whGeoLocation = $warehouse->getWhGeoLocation();
		...
		<div class="container-fluid">
			...
			<div class="row">
				...
				<div class="col-md-10">
					...
					<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post"> <!-- Formulario para introducir los datos del almacén -->
						<div class="form-group">
							<label class="form-label">Warehouse ID</label>
							<input type="text" name="id" class="form-control" required readonly value="<?php echo $warehouse_id; ?>">
                        ...
                        <?php
						echo "<div class=form-group>"; //Select para seleccionar el Lugar del Almacén, enseño la dirección para que sea más fácil seleccionar el Lugar correcto.
							...
							foreach($locations as $location){
                                if($location->getLocationId() == $location_id){
                                    echo "<option value=" . $location_id . " selected>" . $location->getStreetAddress() . "</option>"; //Se selecciona el que tenía
                                }
                                else{
